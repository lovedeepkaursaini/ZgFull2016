...
void BackgroundPrediction(std::string pname,std::string filename, int rebin_factor,int model_number = 0,int imass=750, bool plotBands = false)
{
    rebin = rebin_factor;
//    std::string fname = std::string("../fitFilesMETPT34/") + pname + std::string("/histos_bkg.root");
//    std::string fname = dirname + pname + std::string("/histos_bkg.root");
   std::string fname = std::string(filename);
    ...
    TTree *inNT = (TTree*)file->Get("tree");
    RooRealVar isSR("isSB_ab","isSB_ab",-2,2);

    TH1D *h_mX_CRb=(TH1D*)file->Get("distribs_2_10_1");
    TH1D *h_mX_CRab=(TH1D*)file->Get("distribs_3_10_1");
    TH1D *h_mX_CRabt=(TH1D*)file->Get("distribs_4_10_1");
    TH1D *h_mX_CRabat=(TH1D*)file->Get("distribs_5_10_1");
    TH1D *h_mX_CR_tau;//=(TH1F*)file->Get("distribs_1_10_1")->Clone("CR_tau");
    if(pname=="antibtag")h_mX_CR_tau =(TH1D*) h_mX_CRab->Clone();
    else if(pname=="btag")h_mX_CR_tau = (TH1D*)h_mX_CRb->Clone();
    else if(pname=="tau")h_mX_CR_tau = (TH1D*)h_mX_CRabt->Clone();
    else if(pname=="anti")h_mX_CR_tau = (TH1D*)h_mX_CRabat->Clone();
    ...
    if(pname=="antibtag")h_mX_SR =(TH1D*) h_mX_ab->Clone();
    else if(pname=="btag")h_mX_SR = (TH1D*)h_mX_b->Clone();
    else if(pname=="tau")h_mX_SR = (TH1D*)h_mX_abt->Clone();
    else if(pname=="anti")h_mX_SR = (TH1D*)h_mX_abat->Clone();

    h_mX_SR->SetName("The_SR");
...
*/    
    ...
    if(blind) {
        ...
        h_SR_Prediction->Scale(nEventsSR/h_SR_Prediction->GetSumOfWeights());
    
    }
    h_SR_Prediction->SetMarkerSize(0.5);
...
//    RooDataSet  pred("pred", "Prediction from SB", inNT, RooArgSet(x,isSR),"isSB_at > 0.5");
...
//    aC_plot->GetXaxis()->SetRangeUser(600,1000);
    aC_plot->GetXaxis()->SetRangeUser(SR_lo, SR_hi);
    aC_plot->GetXaxis()->SetLabelOffset(0.02);
    aC_plot->GetYaxis()->SetRangeUser(0.01, 100000.);
    ...
    aC_plot->SetTitle("");
    TPaveText *pave = new TPaveText(0.85,0.65,0.67,0.5,"NDC");
    ...
    if (blind)
     {
        if (blah == "antibtag" )
            leg->AddEntry(h_SR_Prediction, "Data: anti-b-tag SB", "ep");
        else if (blah == "btag" )
            leg->AddEntry(h_SR_Prediction, "Data: b-tag SB", "ep");
        else if(blah == "tau" )
            leg->AddEntry(h_SR_Prediction, "Data: tau SB", "ep");
        else if(blah == "anti" )
            leg->AddEntry(h_SR_Prediction, "Data: anti-tau SB", "ep");
        ...
        if (blah == "antibtag" )
            ...
            leg->AddEntry(h_SR_Prediction, "Data: tau SR", "ep");
        else if(blah == "anti" )
            leg->AddEntry(h_SR_Prediction, "Data: anti-tau SR", "ep");
    ...
    frameP->SetTitle("");
    frameP->GetXaxis()->SetRangeUser(SR_lo, SR_hi);
//    frameP->GetXaxis()->SetRangeUser(600,1000);
