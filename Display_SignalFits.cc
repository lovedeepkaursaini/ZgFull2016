...
//    double rangeLo=TMath::Max(600., massL-0.3*massL), rangeHi=TMath::Min(3600., massL+0.3*massL);
    ...
    sg_p5=new RooRealVar((std::string("sg_p5")+postfix).c_str(), "sg_p5", 150, 0., 30000.);
    ...
    x=new RooRealVar("x", "m_{X} (GeV)", 600., 4600.);
    ...
    if(color==kBlack)signal.fitTo(signalHistogram, RooFit::Hesse(false), RooFit::Range(rangeLo, rangeHi), RooFit::Save());
    else if(color==kRed)signalCore.fitTo(signalHistogram, RooFit::Hesse(false), RooFit::Range(rangeLo, rangeHi), RooFit::Save());
    ...
    if (color==kBlack)
    {
        ...
        RooAddPdf signal_fixed((std::string("sig_fixed_")+postfix).c_str(), "signal", RooArgList(signalCore_fixed, signalComb_fixed), signal_p6);
...
//  Systematics on photon energy scale and resolution
	...
	w->factory("prod::nuisancedsignal_p0_"+TString::Format("%s",postfix.c_str())+"(signal_p0_"+TString::Format("%s",postfix.c_str())+",sumNuis_pes)");
...
//	w->factory("EDIT::signal_fixed_anti(sig_fixed_anti,signal_p0_anti=nuisancedsignal_p0_anti)");
	...
	w->factory("prod::nuis_jer_signal_p1_"+TString::Format("%s",postfix.c_str())+"(signal_p1_"+TString::Format("%s",postfix.c_str())+",sumNuis_swidth_jer)");
	w->factory("EDIT::signal_fixed_"+TString::Format("%s",postfix.c_str())+"(sig_fixed_"+TString::Format("%s",postfix.c_str())+",signal_p0_"+TString::Format("%s",postfix.c_str())+"=nuisancedsignal_p0_"+TString::Format("%s",postfix.c_str())+",signal_p1_"+TString::Format("%s",postfix.c_str())+"=nuis_jer_signal_p1_"+TString::Format("%s",postfix.c_str())+")");
    ...
    else if (color == kRed)
    {
        ...
        RooRealVar signal_p5((std::string("signal_p5_")+postfix).c_str(), "signal_p5", sg_p5->getVal());
        RooRealVar signal_p6((std::string("signal_p6_")+postfix).c_str(), "signal_p6", sg_p6->getVal());
        RooCBShape signal_fixed((std::string("sig_fixed_")+postfix).c_str(), "signal", *x, signal_p0, signal_p1,signal_p2, signal_p3);
        RooWorkspace *w=new RooWorkspace("Vg");
        w->import(signal_fixed);
        w->factory("ggH_hzg_mShift_el_8TeV_cat0[0.0,-5.0,5.0]");
        w->factory("unc_pes[0.01]");
...
//histZg_minitree_mrgd_ZToQQ_W_0-p-014_M_1750.root
        ...
        else if(postfix=="tau")h_mX_SR = (TH1D*)h_mX_abt->Clone();
        else if(postfix=="anti")h_mX_SR = (TH1D*)h_mX_abat->Clone();
        ...
        //RooPlot *plot_vg;
        if(postfix=="anti")RooPlot *plot_vg=fitSignal(dirName,h_mX_SR, imass, masses.at(i), kBlack, leg, params_vg,postfix, true);
        else RooPlot *plot_vg=fitSignal(dirName,h_mX_SR, imass, masses.at(i), kRed, leg, params_vg,postfix, true);
